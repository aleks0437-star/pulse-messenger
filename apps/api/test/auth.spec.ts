...
function harness() {
  ...
  const config: any = {
    ...
    getOrThrow: jest.fn((key: string) =>
      key === "REFRESH_JWT_SECRET" ? process.env.REFRESH_JWT_SECRET : undefined,
    ),
  };
...
describe("AuthService", () => {
  it("registers with a hash, safe response and 15-minute access token", async () => {
    ...
    db.user.create.mockImplementation(({ data }: any) => ({
      ...user,
      ...data,
    }));
    ...
    expect(
      await bcrypt.compare(
        "Secure12345",
        db.user.create.mock.calls[0][0].data.passwordHash,
      ),
    ).toBe(true);
    ...
    expect(db.refreshToken.create.mock.calls[0][0].data.tokenHash).not.toBe(
      result.refreshToken,
    );
  });
  ...
  it("rejects weak registration data through the DTO policy", async () => {
    ...
    expect(errors.map((error) => error.property)).toEqual(
      expect.arrayContaining(["displayName", "password"]),
    );
  });
  ...
  ])("returns a clear conflict for duplicate %s", async (target, message) => {
    const { db, service } = harness();
    db.user.create.mockRejectedValue({ code: "P2002", meta: { target } });
    await expect(
      service.register({
        email: user.email,
  ...
  it("logs in with valid credentials and rejects an invalid password", async () => {
    ...
    await expect(
      service.login({ login: "anna", password: "Secure12345" }),
    ).resolves.toHaveProperty("accessToken");
    await expect(
      service.login({ login: "anna", password: "wrong" }),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  });

  it("performs bcrypt comparison even when the user does not exist", async () => {
    const { db, service } = harness();
    db.user.findFirst.mockResolvedValue(null);
    await expect(
      service.login({ login: "missing", password: "wrong" }),
    ).rejects.toBeInstanceOf(UnauthorizedException);
  ...
  it("rotates a refresh token and revokes the previous record", async () => {
    ...
    expect(db.refreshToken.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: { id: initialRecord.id, revokedAt: null },
  ...
  it("revokes refresh token on logout without storing the raw token", async () => {
    const { db, service } = harness();
    await expect(
      service.logout("a-valid-looking-refresh-token-value"),
    ).resolves.toEqual({ ok: true });
    const where = db.refreshToken.updateMany.mock.calls[0][0].where;
    expect(where.tokenHash).toMatch(/^[a-f0-9]{64}$/);
    expect(
      JSON.stringify(db.refreshToken.updateMany.mock.calls[0][0]),
    ).not.toContain("a-valid-looking-refresh-token-value");
  });
});
